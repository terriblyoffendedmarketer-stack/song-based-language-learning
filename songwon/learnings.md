...
- TTS on Vercel fails with API route | `readFile` and `execFile(python3)` don't work on Vercel serverless functions — no filesystem for cached MP3s, no Python runtime | Move TTS MP3s to `public/tts/`, serve as static files, client resolves text→hash via `lookup.json`
- English text default violates Korean-first principle | User wants maximum target language exposure — all UI text should be Korean by default | Wrap every UI string in KrTip (Korean primary, English on tap)
- Practice must never be gated behind lessons | Practice is the retention engine — it should always be available with all vocab. Lessons gate behind practice sessions, not the reverse | Load all vocab regardless of `completedLessons`
